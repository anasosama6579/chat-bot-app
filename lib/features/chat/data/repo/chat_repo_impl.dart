  ...
  Future<ApiResult<ChatResponse>> chat(ChatRequestBody chatRequestBody) async {
    if (chatRequestBody.contents == null || chatRequestBody.contents!.isEmpty) {
      return ApiResult.failure(
        ErrorHandler.handle(Exception("Request cannot be empty.")),
      );
    }
    try {
      ...
      if (response.candidates == null || response.candidates!.isEmpty) {
        return ApiResult.failure(
          ErrorHandler.handle(
            Exception("Received empty candidates from the server."),
          ),
        );
      }
      ...
      if (responseText == null || responseText.isEmpty) {
        return ApiResult.failure(
          ErrorHandler.handle(
            Exception("Received an empty message from the server."),
          ),
        );
      }
